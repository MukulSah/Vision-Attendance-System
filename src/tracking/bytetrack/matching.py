...
def bbox_ious(boxes1, boxes2):
    ...
    for i in range(N):
        ...
        for j in range(M):
            xx1 = max(x1, boxes2[j][0])
            yy1 = max(y1, boxes2[j][1])
            xx2 = min(x2, boxes2[j][2])
            yy2 = min(y2, boxes2[j][3])

            w = max(0, xx2 - xx1)
            h = max(0, yy2 - yy1)
            inter = w * h

            area2 = max(0, boxes2[j][2] - boxes2[j][0]) * \
                    max(0, boxes2[j][3] - boxes2[j][1])
